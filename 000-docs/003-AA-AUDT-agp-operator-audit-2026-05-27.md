...
The system AGP composes is not theoretical. The **governance kernel** lives in the `claude-code-slack-channel` repo (CCSC), shipped at v0.10.0 on 2026-05-24 after a 17-PR rollout. CCSC has 986 tests across 17 production TypeScript modules totaling ~12k LoC of production code, runs as a single-binary Bun process, talks to Slack via Socket Mode, exposes an MCP-stdio surface to Claude Code, and has a battle-tested security boundary model documented in six load-bearing design docs (`THREAT-MODEL.md`, `session-state-machine.md`, `policy-evaluation-flow.md`, `audit-journal-architecture.md`, `bot-manifest-protocol.md`, `ARCHITECTURE.md`).
...
| Total production TS LoC | ~13,000 | sum of `*.ts` excluding test files |

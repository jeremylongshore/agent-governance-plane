...
| 4 — State & Memory | Remember only what matters | GSB — now Bob's Big Brain (ICO `intentional-cognition-os` + INTKB `qmd-team-intent-kb` + qmd; plugin `bobs-big-brain-plugin`) — hash-chained receipts | Compose; do not build a new state store |

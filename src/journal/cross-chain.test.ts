...
import { reconstructKnowledgeAt } from "./cross-chain.ts";
...
// Acceptance criterion (agp-eva.1.2): given a governed run, the AGP journal can
// reconstruct "what the agent knew at action X" via the shared correlation_id and the
// embedded GSB receipt tip-hash — the pointers a GSB audit then resolves.
test("reconstructs a run's actions + the GSB receipt tips observed at each decision", () => {
  ...
  const run = reconstructKnowledgeAt(readEvents(path), "run-A");
...
test("dedupes repeated GSB tips first-seen, and null tips do not enter the pointer list", () => {
  ...
  const run = reconstructKnowledgeAt(readEvents(path), "run-C");
...
test("an unknown correlation id yields an empty (but well-formed) reconstruction", () => {
  ...
  const run = reconstructKnowledgeAt(readEvents(path), "run-NONE");

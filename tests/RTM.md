...
| REQ-044 | **Cross-chain governance-pointer primitive (invariant 1):** the signed journal carries a shared `correlationId` plus an observed external governance-chain tip hash. The fields make action-to-chain-position correlation reconstructable offline; they do not identify an exact knowledge read set or establish causation. (The trigger-woken agent's `trigger.fired`/`trigger.settled` bracketing that consumes it lives in `iam-bob-intendant`.) | MUST | 058-AT-ADR; 056-AT-CONT (correlationId REQUIRED); `src/journal/cross-chain.ts` | `src/journal/cross-chain.test.ts`; `src/contracts/trigger-source.test.ts` :: "TriggerEvent REQUIRES correlationId (cross-chain causal pointer invariant)" | ✓ Covered |

...
**Sources:** `000-docs/001-AT-DECR` (10 locked P0 decisions), `000-docs/002-PP-PLAN` (master blueprint: 4 defining properties §1, trust boundaries §2.2, threat model §4), `000-docs/009-AT-ADR` (CCSC substrate extraction), `000-docs/012-AT-SPEC` (CLI surface), `000-docs/013–018` (contract specs), `000-docs/019–027` (Docker sandbox, HITL flow, journal/audit-chain, policy integration, claude-code sprite), `src/contracts/`, `src/**/*.test.ts` (16 files, 87 tests).
...
| REQ-014 | **Reserved schema slots present and null at v0** (`tenant_id`, `signing_key_id`, `approval_binding_type`, `sprite_identity_uri`) — CISO non-negotiable, locking now so activation is not a breaking change. | MUST | 001-AT-DECR Q2 binding constraint; 023-AT-SPEC §Reserved | `src/contracts/journal-event.test.ts` :: "reserves all four future fields, present and null at v0 (council lock)"; "reserved fields default to null when omitted (forward-compatible slot)" | ✓ Covered |
...
| REQ-018 | **Gate-only mediation for the live harness (no proxy-exec):** the Claude Code sprite passes each read/write/shell/git call through the gate and journals it; the harness executes its own tools — AGP never proxy-executes. A denied call returns allow=false with a reason and the harness keeps running; an unmatched call is default-denied at the gate. | MUST | 027-AT-SPEC (claude-code sprite §Gate-only); 016-AT-CONT (sprite-adapter); 002-PP-PLAN §2.2 | `src/sprites/claude-code/claude-code-sprite.test.ts` :: "read/write/shell/git calls each pass through the gate and are journaled, gate-only (no proxy-exec)"; "a denied call is blocked: the hook receives allow=false with the reason, harness keeps running"; "an unmatched call is default-denied (fail-closed) at the gate" | ✓ Covered |
...
| REQ-029 | **SpriteAdapter contract conformance:** a reference SpriteAdapter conforms and drives a session; SpriteIdentity parses with `uri` reserved (null) at v0. | SHOULD | 016-AT-CONT; 027-AT-SPEC; `src/contracts/sprite-adapter.ts` | `src/contracts/behavioral.test.ts` :: "a reference SpriteAdapter conforms and drives a session"; "SpriteIdentity parses; uri reserved (null) at v0" | ✓ Covered |
...
| REQ-033 | **Session-id binding for run:** `run(sessionId)` must match the started session; `stop()` terminates the session — pending hooks unblock as denied and remaining calls do not run. | SHOULD | 027-AT-SPEC; 016-AT-CONT | `src/sprites/claude-code/claude-code-sprite.test.ts` :: "run(sessionId) must match the started session"; "stop() terminates the session: pending hooks unblock as denied and remaining calls do not run" | ✓ Covered |
...
| REQ-036 | **Live spawn is gated behind an opt-in env flag** (`AGP_CLAUDE_LIVE`) so the default path is deterministic/offline. | COULD | 027-AT-SPEC | `src/sprites/claude-code/claude-code-sprite.test.ts` :: "BunClaudeProcess live spawn is gated behind AGP_CLAUDE_LIVE" | ✓ Covered |
...
| `src/sprites/claude-code/claude-code-sprite.test.ts` | REQ-018, 021, 022, 033, 036 |
